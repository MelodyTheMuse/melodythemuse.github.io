...
function createMediaContent(project) {
	if (project.video) {
		...
		const images = project.images.map((img, index) => `
            <img src="${img}" alt="Project image ${index + 1}" class="project-media ${index === 0 ? 'active' : ''}">`).join('');
...
fetch("./projects.json")
	...
	.then(projects => {
		...
		// Find a highlighted project and other projects
		...
		// Add a highlighted project if it exists
                ...
                <div class="project-content">
                    ...
                    ${highlightedProject.category ? `
                        <span class="project-category ${highlightedProject.category.toLowerCase().replace(/\s+/g, '-')}">
                            ${highlightedProject.category}
                            ${highlightedProject.category === 'Internship' && highlightedProject.company ? ` - ${highlightedProject.company}` : ''}
                        </span>
                    ` : ''}
                ...
                <div class="project-content">
                    ...
                    ${project.category ? `
                        <span class="project-category ${project.category.toLowerCase().replace(/\s+/g, '-')}">
                            ${project.category}
                            ${project.category === 'Internship' && project.company ? ` - ${project.company}` : ''}
                        </span>
                    ` : ''}
...
function updateProjectVisibility() {
	...
	projects.forEach(project => {
		...
		const hasAllFilters = Array.from(selectedFilters).every(filter => technologies.includes(filter));
